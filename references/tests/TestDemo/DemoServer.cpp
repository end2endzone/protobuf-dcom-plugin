...
// AppID
const char g_szAppID[] = "{F3DB80E7-E820-401A-9C69-5CA27F007BED}";
...
///<GENERATEDCODE>
STDMETHODIMP CoDemoServer::FooFunction(VARIANT requestBuffer)
{
  testdemo::VariantFlavor flavor;
  dcomplugin::Identifier requestMessage;
  ...
  testdemo::FooResponse responseMessage;
  g_objDemoServerImpl.FooFunction(requestMessage);

  return S_OK;
}

STDMETHODIMP CoDemoServer::GetSequentialNumber(VARIANT* responseBuffer)
{
  testdemo::VariantFlavor flavor = testdemo::CPLUSPLUS;

  //call server implementation
  testdemo::SequentialNumber responseMessage;
  g_objDemoServerImpl.GetSequentialNumber(responseMessage);
